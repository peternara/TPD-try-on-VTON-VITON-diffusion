...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        ...
        GT_mask = GT_mask.to(memory_format=torch.contiguous_format).float()
        inpaint_image = inpaint_image.to(memory_format=torch.contiguous_format).float()
        inpaint_mask = inpaint_mask.to(memory_format=torch.contiguous_format).float()
        posemap = posemap.to(memory_format=torch.contiguous_format).float()
        densepose = densepose.to(memory_format=torch.contiguous_format).float()
        ref_list[0] = ref_list[0].to(memory_format=torch.contiguous_format).float()

        return GT_image, GT_mask, inpaint_image, inpaint_mask, posemap, densepose, ref_list
...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # PDE와 달리 condition 부분이 없는 알고리즘이므로 제거
    ...
    def get_input(
        ...
        GT_image, GT_mask, inpaint_image, inpaint_mask, posemap, densepose, ref_list = super().get_input(batch, k)


        if bs is not None:
            GT_image = GT_image[:bs]
            GT_mask = GT_mask[:bs]
            inpaint_image = inpaint_image[:bs]
            inpaint_mask = inpaint_mask[:bs]
            posemap = posemap[:bs]
            densepose = densepose[:bs]
            ref_list[0] = ref_list[0][:bs]

        GT_image = GT_image.to(self.device)
        GT_mask = GT_mask.to(self.device)
        inpaint_image = inpaint_image.to(self.device)
        inpaint_mask = inpaint_mask.to(self.device)
        posemap = posemap.to(self.device)
        densepose = densepose.to(self.device)
        ref_list[0] = ref_list[0].to(self.device)


        def VAE_Encode(*image_list):
            latents = []
            for image in image_list:
                encoder_posterior = self.encode_first_stage(image)  # 获取分布
                z = self.get_first_stage_encoding(encoder_posterior).detach()  # 采样
                latents.append(z)
            return latents
        z_GT_image, z_inpaint_image, z_posemap, z_densepose = VAE_Encode(GT_image, inpaint_image, posemap, densepose)
        
        z_shape = (z_GT_image.shape[-2], z_GT_image.shape[-1])
        z_GT_mask = Resize([z_shape[0], z_shape[1]])(GT_mask)
        z_inpaint_mask = Resize([z_shape[0], z_shape[1]])(inpaint_mask)

        z_new = torch.cat((z_GT_image,z_GT_mask,z_inpaint_image,z_inpaint_mask,z_posemap,z_densepose),dim=1)


        out = z_new


        if return_first_stage_outputs or return_original_cond:
            out = [out]

            if return_first_stage_outputs:
                
                if self.first_stage_key == "inpaint":
                    GT_image_recon = self.decode_first_stage(z_new[:, :4, :, :])
                else:
                    xrec = self.decode_first_stage(z_new)
                out.extend([GT_image, GT_image_recon])

            if return_original_cond:
                out.append(GT_mask)
                out.append(inpaint_image)
                out.append(inpaint_mask)
    ...
    def p_losses(
        ...
        if self.first_stage_key == "inpaint":
            noise = default(noise, lambda: torch.randn_like(x_start[:, :5, :, :]))
            x_noisy = self.q_sample(x_start=x_start[:,:5,:,:], t=t, noise=noise)
            x_noisy = torch.cat(
                (x_noisy, x_start[:, 5:, :, :]), dim=1
            )
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        if ddim:
            ...
            shape = (self.channels, kwargs["rest"].shape[2], kwargs["rest"].shape[3])
    ...
    def log_images(
        ...
        **kwargs,
        ...
        z, GT_image, GT_image_recon, GT_mask, inpaint_image, inpaint_mask, ref, posemap, densepose = self.get_input(batch, 
        ...
        N = min(GT_image.shape[0], N)
        n_row = min(GT_image.shape[0], n_row)
        log["GT_image"] = GT_image
        log["GT_image_recon"] = GT_image_recon
        log["GT_mask"] = GT_mask
        log["inpaint_image"] = inpaint_image
        log["inpaint_mask"] = inpaint_mask
        log["posemap"] = posemap
        ...
        if self.model.conditioning_key is not None:
            ...
            if ismap(ref):
                log["original_conditioning"] = self.to_rgb(ref)
        ...
        if sample:
            ...
            samples_image = self.decode_first_stage(samples[:, :4, :, :])
            samples_shape = [samples_image.shape[-2], samples_image.shape[-1]]
            samples_mask = Resize(samples_shape)(samples[:, 4, :, :])

            log["samples_image"] = samples_image
            log["samples_mask"] = samples_mask.unsqueeze(1)
            if plot_denoise_rows:
                # denoise_grid = self._get_denoise_row_from_list(z_denoise_row)
                denoise_grid = self._get_denoise_row_from_list(z_denoise_row["pred_x0"])
                log["pred_x0"] = denoise_grid
